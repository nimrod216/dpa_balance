...
class RegressionDataset:
    def __init__(self, shape, testset_size, trainset_size, data_csv):
        # Basic Dataset Info
        self._shape = shape
        self._testset_size = testset_size
        self._trainset_size = trainset_size
    ...
    def name(self):
        assert self.__class__.__name__ != 'RegressionDataset'
        return self.__class__.__name__
...
class DPA_datasets(RegressionDataset):
    def __init__(self, csv_file, factor=1):
        super().__init__(shape=(256),
                         testset_size=1000,
                         trainset_size=7938,
                         data_csv=csv_file)

        self.csv = np.loadtxt(csv_file, dtype=np.str, delimiter=',', skiprows=1)
        self.len = self.csv.shape[0]
        inputs = self.csv[:, :2]
        self.inputs = torch.from_numpy(np.asarray([np.unpackbits(np.array(bytearray.fromhex(i[0] + i[1]))) for i in inputs], dtype=float)).float()
        self.inputs = torch.reshape(self.inputs, [list(self.inputs.size())[0], 1] + list(self.inputs.size())[1:])
        self.outputs = torch.from_numpy(np.asarray(self.csv[:, 2:-1], dtype=float)).float()
        self.outputs = torch.reshape(self.outputs, [list(self.outputs.size())[0], 1] + list(self.outputs.size())[1:])*factor


    def trainset(self, batch_size, device='cuda'):
        # return torch.utils.data.DataLoader(CustomDataset(self.inputs[:7938, :], self.outputs[:7938, :]))
        return torch.utils.data.DataLoader(CustomDataset(self.inputs[:7938, :, :], self.outputs[:7938, :, :]),
